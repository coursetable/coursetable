...
(function() {
    ...
    */
    ...
    {
		for (var key in a) {
			if (a.hasOwnProperty(key) && a[key] === v) {
				return key;
			}
		}
        return -1;
    ...
    */
    ...
    var buildInFunctions = 
        {
            ...
            'if': 
            {
                ...
                process: function(node, data)
                {
                    ...
                    if (value &&
                        !((value instanceof Array && value.length == 0) || // 0 length arrays false in PHP
                        (typeof value == 'object' && isEmptyObject(value)))) // Empty associative arrays false in PHP
                    {
    ...
    function parseParams(paramsStr, reDelim, reName)
    {
        ...
        var params = {};
        params.__parsed = {};
		var highestIndex = -1; // For numeric indices
        var paramsStr = '';
        ...
        while (s)
        {
            ...
            {
				if (parseInt(nm) == nm) {
					highestIndex = Math.max(nm, highestIndex);
				}
            ...
            {
                params[++highestIndex] = param.value;
                params.__parsed[highestIndex] = param.tree;
            }
    ...
    */
    ...
    jSmart.prototype.registerPlugin(
        ...
        function(params, data)
        {
			...
			for (var key in params) {
				// Exclude 'name' and '__get'
				if (params.hasOwnProperty(key) && key != 'name' && key.indexOf('__') != 0) {
					paramNames.push(params.name+key+'__p'+key);
					paramValues[params.name+key+'__p'+key] = params[key];
					i++;
				}
			}
            var fname = ('__owner' in data && params.name in data.__owner) ? ('__owner.'+params.name) :
                ("modifiers['" + params.name.replace(/'/g, "\\'") + "']");
    ...
    jSmart.prototype.registerPlugin(
        ...
        function(params, data)
        {
			var str = '';
			for (var key in params) {
				if (params.hasOwnProperty(key) && key != '__get') {
					str += params[key];
				}
			}
			return str;
