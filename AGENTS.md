...
- Base PRs on `master`.
- Use **lowercase conventional commits**, e.g. `fix: ...`, `feat: ...`, `chore: ...`, `test: ...`, `docs: ...`.
- Prefer one logical change per PR.
- PR descriptions must follow [`.github/PULL_REQUEST_TEMPLATE.md`](.github/PULL_REQUEST_TEMPLATE.md): fill **Summary** and **Test plan (required)**; complete the pre-flight checklist when it applies. Small/straightforward changes may skip the template as noted there.
- Link issues with `Fixes #1234` (or Sentry short IDs in Related) when relevant.
...
- Frontend: React function components, existing Zustand/Apollo patterns. API: Express handlers + Drizzle; reuse existing error code strings (`ALREADY_BOOKMARKED`, etc.) rather than inventing new ones casually.
