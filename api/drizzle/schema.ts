import { mysqlTable, mysqlSchema, AnyMySqlColumn, char, tinyint, varchar, int, index, unique } from "drizzle-orm/mysql-core"
...
export const studentFriendRequests = mysqlTable("StudentFriendRequests", {
...
(table) => {
	return {
		...
		netIdFriendNetId: unique("netId_friendNetId").on(table.netId, table.friendNetId),
	}
});

export const studentFriends = mysqlTable("StudentFriends", {
	id: bigint("id", { mode: "number" }).autoincrement().notNull(),
	netId: char("netId", { length: 8 }).notNull(),
	friendNetId: char("friendNetId", { length: 8 }).notNull(),
},
...
export const worksheetCourses = mysqlTable("WorksheetCourses", {
...
(table) => {
	return {
		netId: index("netId").on(table.netId),
		netIdCrnSeasonWorksheetNumber: unique("netId_crn_season_worksheetNumber").on(table.netId, table.crn, table.season, table.worksheetNumber),
	}
});