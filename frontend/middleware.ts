...
export default async function middleware(req: Request) {
  ...
  const isBot =
      /facebook|linkedin|twitter|pinterest|discord|bing|google|whatsapp|slack|vercel\sedge\sfunctions/iu
          .test(
              userAgent,
          );
  ...
  const {data} = (await response.json()) as {
    data?: {
      computed_listing_info: {
        course_code: string; section: string; title: string;
  ...
  return new Response(
      ...
      <html>
        <head>
          ...
          <meta property="og:image" content="https://coursetable.com/api/opengraph/${
          encodedTitle}" />
