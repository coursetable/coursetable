...
const EvaluationResponses: React.FC<{
  ...
  const [responses, sortedResponses] = useMemo(() => {
    ...
    (info || []).forEach((section) => {
      ...
      nodes.forEach((node) => {
        const questionText = node.evaluation_question.question_text;
        if (questionText && node.comment) {
          if (!tempResponses[questionText]) tempResponses[questionText] = [];
          tempResponses[questionText].push(node.comment);
        }
      });
    });
    const tempSortedResponses = JSON.parse(
      JSON.stringify(tempResponses),
    ) as typeof tempResponses;
    for (const key of Object.keys(tempResponses)) {
      tempSortedResponses[key].sort((a, b) => b.length - a.length);
    }
    return [tempResponses, tempSortedResponses];
  }, [info, crn]);
  ...
  const [recommend, skills, strengths, summary] = useMemo(() => {
    // Lists that hold the elements for the comments for a specific question
